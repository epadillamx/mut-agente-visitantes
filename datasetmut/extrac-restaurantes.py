...
for i in range(1, 101):
    url = f"https://mut.cl/wp-json/wp/v2/restaurant?per_page=100&page={i}"
    payload = {}
    headers = {}
...
if todas_tiendas:
    ...
    df.to_csv(r"C:\gitkraken\SISGEST\AGENTE-AWS\todas_restaurantes.csv", index=False, encoding='utf-8-sig')

    print(f"\n{'=' * 60}")
    print(f"✓ TOTAL: {len(df)} tiendas guardadas en todas_restaurantes.csv")
    print(f"{'=' * 60}")
    print(f"\nPrimeras 5 tiendas:")
else:
